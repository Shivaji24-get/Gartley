...
with tab_scan:
    ...
    if not st.session_state.has_scanned:
        ...
        st.divider()
        st.subheader("4. Pattern details (X-A-B-C-D)")
        st.caption("The exact swing points and Fibonacci values the scanner calculated for "
                   "each match — shown as-is, for verification against the underlying code.")

        for c in st.session_state.scan_results:
            with st.expander(f"{c.symbol} — {'BUY' if c.direction == 'bullish' else 'SELL'} pattern "
                              f"({'complete' if c.setup_complete else 'forming'})"):

                st.markdown("**Swing points**")
                points_df = pd.DataFrame([
                    {"Point": "X", "Date": c.X.date.strftime("%Y-%m-%d %H:%M"),
                     "Price": f"₹{c.X.price:,.2f}", "Type": c.X.kind},
                    {"Point": "A", "Date": c.A.date.strftime("%Y-%m-%d %H:%M"),
                     "Price": f"₹{c.A.price:,.2f}", "Type": c.A.kind},
                    {"Point": "B", "Date": c.B.date.strftime("%Y-%m-%d %H:%M"),
                     "Price": f"₹{c.B.price:,.2f}", "Type": c.B.kind},
                    {"Point": "C", "Date": c.C.date.strftime("%Y-%m-%d %H:%M"),
                     "Price": f"₹{c.C.price:,.2f}", "Type": c.C.kind},
                    {"Point": "D (cluster level)",
                     "Date": c.touch_date.strftime("%Y-%m-%d %H:%M") if c.touch_date is not None else "Not touched yet",
                     "Price": f"₹{c.cluster_price:,.2f}", "Type": "projected/touched"},
                ])
                st.dataframe(points_df, hide_index=True, width="stretch")

                col_a, col_b = st.columns(2)
                with col_a:
                    st.markdown("**XA Fibonacci retracement levels**")
                    retr_df = pd.DataFrame([
                        {"Level": f"{lvl*100:.1f}%", "Price": f"₹{price:,.2f}",
                         "Matched cluster?": "Yes" if abs(lvl - c.cluster_level) < 1e-9 else ""}
                        for lvl, price in sorted(c.xa_retracements.items())
                    ])
                    st.dataframe(retr_df, hide_index=True, width="stretch")

                with col_b:
                    st.markdown("**AB = CD extension & cluster**")
                    st.write(f"Extension ratio used: **{c.extension_ratio*100:.0f}%**")
                    st.write(f"AB = CD projected price: **₹{c.extension_D:,.2f}**")
                    st.write(f"Closest XA retracement level: **{c.cluster_level*100:.1f}%** "
                             f"(₹{c.cluster_price:,.2f})")
                    st.write(f"Distance between extension and retracement: "
                             f"**{c.cluster_distance_pct:.2f}%** of the XA range")
                    st.write(f"Setup status: **{'Complete — price touched the retracement level' if c.setup_complete else 'Forming — not yet touched'}**")
                    st.write(f"Current price: **₹{c.current_price:,.2f}** "
                             f"(as of {c.latest_candle_date.strftime('%Y-%m-%d %H:%M')})")

        with st.expander("View pattern points (X, A, B, C, D) for each stock"):
            pattern_rows = []
            for c in st.session_state.scan_results:
                pattern_rows.append({
                    "Stock": c.symbol,
                    ...
                    "B": f"₹{c.B.price:,.2f}",
                    "C": f"₹{c.C.price:,.2f}",
                    "D (completion level)": f"₹{c.cluster_price:,.2f}",
                })
            st.dataframe(pd.DataFrame(pattern_rows), hide_index=True, width="stretch")
...
with tab_signals:
    ...
    for c in complete_clusters:
        ...
        with st.container(border=True):
            ...
            st.caption(f"Pattern points — X: ₹{c.X.price:,.2f} · A: ₹{c.A.price:,.2f} · "
                       f"B: ₹{c.B.price:,.2f} · C: ₹{c.C.price:,.2f} · "
                       f"D (completion level): ₹{c.cluster_price:,.2f}")
    ...
    for sym, pos in om.open_positions.items():
        with st.container(border=True):
            ...
            with st.expander("Trade log"):
                st.caption(f"Pattern points — X: ₹{pos.X_price:,.2f} · A: ₹{pos.A_price:,.2f} · "
                           f"B: ₹{pos.B_price:,.2f} · C: ₹{pos.C_price:,.2f} · "
                           f"D (completion level): ₹{pos.D_price:,.2f}")
                if pos.history:
